...
# Глобальные переменные для хранения менеджеров
# Они будут установлены в app.py
_manager_obj = None
_processor_obj = None

def set_managers(manager_obj, processor_obj):
    """Установка менеджеров из app.py"""
    global _manager_obj, _processor_obj
    _manager_obj = manager_obj
    _processor_obj = processor_obj

# Функции для HTTP endpoints (используют Depends)
...
# WebSocket endpoint не может использовать Depends, поэтому напрямую используем глобальные переменные
@router.websocket("/ws/analytics/{client_id}")
async def websocket_endpoint(websocket: WebSocket, client_id: str):
    # Используем глобальные менеджеры
    if _manager_obj is None or _processor_obj is None:
        await websocket.close(code=1011)  # Внутренняя ошибка сервера
        return
    
    await _manager_obj.connect(websocket, client_id)
    ...
    try:
        while True:
            ...
            try:
                ...
                await _processor_obj.process_action(action, _manager_obj)
        ...
        _manager_obj.disconnect(client_id)
...
async def get_user_stats(
    ...
    try:
        ...
        # Исправление: для redis.asyncio пайплайн не используем with
